...
public class AutoTestGeneratorServiceImpl implements AutoTestGeneratorService {
	...
	public File createPostmanCollection(MultipartFile apiSpec, MultipartFile dataFile) throws Exception {
		...
		/*
		 * StringBuffer rawCollection = new StringBuffer(System.getProperty("user.dir"))
		 * .append("/").append("output") .append("/").append(uuid)
		 * .append("/").append("collection.json"); String rawCollectionFile =
		 * rawCollection.toString();
		 */

		command = "openapi2postmanv2 -s " + apiSpecFile.getAbsolutePath() + " -o collection.json";
		// command = "openapi2postmanv2 -s " + apiSpecFile.getAbsolutePath() + " -o " +
		// rawCollectionFile;
		if (System.getProperty("os.name").toLowerCase().contains("windows")) {
			command = new StringBuilder("cmd.exe /c ").append(command).toString();
		} else {
		...
		/*
		 * StringBuffer modifiedCollection = new
		 * StringBuffer(System.getProperty("user.dir")) .append("/").append("output")
		 * .append("/").append(uuid) .append("/").append("ModifiedJson.json"); String
		 * modifiedCollectionFile = modifiedCollection.toString(); File fileOp = new
		 * File(rawCollectionFile); boolean isModified = addTestsToRequest(fileOp,
		 * apiDataFile); System.out.println("isModified: " + isModified); if
		 * (isModified) { fileOp = new File("ModifiedJson.json"); }
		 */
		...
		FileUtils.deleteDirectory(outputFolder); // To be handled in a appropriate way.
		return fileOp;
	}
	...
	public File runPostmanCollection(MultipartFile collectionJson, MultipartFile dataFile) throws Exception {
		...
		command = "newman run " + postmanCollection.getAbsolutePath() + " " + " -d " + testDataFile.getAbsolutePath()
				+ " -r htmlextra";
	...
	@Override
	public File createPostmanCollectionWithDataMap(MultipartFile apiSpec, Map<String, Object> apiDataMap)
			throws Exception {
	...
	private JsonNode updateDataFromCSVAtNodeValue(JsonNode node, Map<String, Object> apiDataMap, String nodeValue) {
		...
		if (node.get("value").asText().contains(nodeValue)) {
			...
			if (apiDataMap.containsKey(key)) {
				...
				ObjectNode queryParameter = (ObjectNode) node;
				String value = (String) apiDataMap.get(key);
	...
	private JsonNode updateDataFromMap(JsonNode node, Map<String, Object> apiDataMap) {
		// if (node.get("value").asText().contains("<") ||
		// node.get("value").asText().contains("NaN")) {
	...
	public File runPostmanCollection(MultipartFile collectionJson, Map<String, Object> apiData) throws Exception {
		...
		// TODO Logic to be added to update the uploaded or attached file with provided
		// Key-Value pairs

		// HashMap<String, Object> apiDataMap =
		// jsonStringToHashMap(apiData.get("apiData").toString());
		HashMap<String, Object> apiDataMap = (HashMap<String, Object>) apiData;

		// File fileOp = new File("collection.json");
		if (addTestsToRequest(postmanCollection, apiDataMap)) {
		...
		command = "newman run " + postmanCollection.getAbsolutePath() + " "
	...
	@Override
	public File createPostmanCollectionWithDataMap(MultipartFile apiSpec, Map<String, Object> apiData,
		...
		if (files != null) {
			if (!files.isEmpty()) {
				for (MultipartFile file : files) {
					File storedFile = multipartFileToFile(file, outputFolder.toPath());
					filesMap.put(storedFile.getName().split("\\.")[0], storedFile.getAbsolutePath());
				}
			}
		}
		...
		if (files != null) {
			if (!files.isEmpty()) {
				if (addTestsToRequest(fileOp, apiData, filesMap)) {
					fileOp = new File("ModifiedJson.json");
				}
			} else {
				if (addTestsToRequest(fileOp, apiData)) {
					fileOp = new File("ModifiedJson.json");
				}
			}
		} else {
	...
	private boolean addTestsToRequest(File jsonFile, Map<String, Object> apiDataMap, Map<String, Object> filesMap) {
		boolean fileModified = true;
		ObjectMapper objectMapper = new ObjectMapper();
		try {
			JsonNode rootNode = objectMapper.readTree(jsonFile);
			// Updating global variables if any
			JsonNode updatedJsonNode = updateGlobalVariables(rootNode, apiDataMap);
			addTestInEvent(updatedJsonNode, apiDataMap, filesMap);
			// addTestInEvent(rootNode, apiDataMap);
		} catch (Exception e) {
			System.out.println(e.getMessage());
			fileModified = false;
	...
	private void addTestInEvent(JsonNode jsonNode, Map<String, Object> apiDataMap, Map<String, Object> filesMap)
			throws StreamWriteException, DatabindException, IOException {
		System.out.println("addTestInEvent: 3 params");
		JsonNode nxtNode = jsonNode.get("item");
		for (JsonNode arrNode : nxtNode) {
			if (arrNode.has("item")) {
				addTestInEvent(arrNode, apiDataMap, filesMap);
			} else {
				...
				if (filesMap.size() > 0) {
					// Update file's value\src
					System.out.println("\\nChecking mode for files..");
					if (arrNode.get("request").get("body") != null) {
						if (arrNode.get("request").get("body").has("mode")) {
							String bodyMode = arrNode.get("request").get("body").get("mode").asText();
							if (!bodyMode.equalsIgnoreCase("raw")) {
								// Update body
								for (JsonNode variables : arrNode.get("request").get("body").get(bodyMode)) {
									System.out.println("Inside setting file..");
									variables = updateDataFromMapForFiles(variables, filesMap);
								}
							}
						}
					}
	...
	private JsonNode updateDataFromMapForFiles(JsonNode node, Map<String, Object> apiDataMap) {
		...
		if (apiDataMap.containsKey(key)) {
			...
			String value = (String) apiDataMap.get(key); // completeAPIData.get(row)[count];
			System.out.println("Reached at desired code..");
			if (node.get("type").asText().equalsIgnoreCase("file")) {
				queryParameter.remove("src");
				queryParameter.put("src", value);
			} else {
				queryParameter.remove("value");
				queryParameter.put("value", value);
			}
	...
	public static HashMap<String, Object> jsonStringToHashMap(String jsonString) {
		...
		// Define type of HashMap
		Type type = new com.google.gson.reflect.TypeToken<HashMap<String, Object>>() {
		}.getType();
	...
	@Override
	public File runPostmanCollection(MultipartFile collectionJson, Map<String, Object> apiData,
			List<MultipartFile> dataFiles) throws Exception {
		...
		// HashMap<String, Object> apiDataMap = jsonStringToHashMap(apiData.get("apiData").toString());
		// HashMap<String, Object> apiDataMap = (HashMap<String, Object>) apiData;
		...
		if (dataFiles != null) {
			if (dataFiles.isEmpty()) {
				...
				if (addTestsToRequest(postmanCollection, apiData, filesMap)) {
					postmanCollection = new File("ModifiedJson.json");
				}
			}
		} else {
