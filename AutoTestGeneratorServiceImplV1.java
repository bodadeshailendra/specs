...
public class AutoTestGeneratorServiceImpl implements AutoTestGeneratorService {
	...
	public File createPostmanCollectionWithDataMap(MultipartFile apiSpec, Map<String, Object> apiDataMap,
		...
		if (files != null && !files.isEmpty()) {
			if (addTestsToRequest(fileOp, apiDataMap, filesMap)) {
				fileOp = new File("ModifiedJson.json");
			}
		} else {
			if (addTestsToRequest(fileOp, apiDataMap)) {
	...
	private boolean addTestsToRequest(File jsonFile, Map<String, Object> apiDataMap, Map<String, String> filesMap) {
		boolean fileModified = true;
		ObjectMapper objectMapper = new ObjectMapper();
		try {
			JsonNode rootNode = objectMapper.readTree(jsonFile);
	...
	private void addTestInEvent(JsonNode jsonNode, Map<String, Object> apiDataMap, Map<String, String> filesMap)
			throws StreamWriteException, DatabindException, IOException {
	...
	private JsonNode updateDataFromFilesMap(JsonNode node, Map<String, String> filesMap) {
		...
		if (filesMap.containsKey(key)) {
			...
			String value = (String) filesMap.get(key); // completeAPIData.get(row)[count];
			queryParameter.remove("value");
			queryParameter.put("value", value);
		}
		return node;
	...
	private JsonNode updateDataFromMapForFiles(JsonNode node, Map<String, String> apiDataMap) {
